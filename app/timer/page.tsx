...
export default function TimerPage({ backButtonCallback }: {
    ...
    <div className="recipe-root">
      <div style={{ width: 1045, margin: "auto", }}>
        ...
        <div className="number-display">
          <div><div>{pad2(timeParts.hours)}</div><div>Hour</div></div>
          <div><div>:</div></div>
          <div><div>{pad2(timeParts.minutes)}</div><div>Minutes</div></div>
          <div><div>:</div></div>
          <div><div>{pad2(timeParts.seconds)}</div><div>Seconds</div></div>
        </div>
